...
// Server-managed fields (`id` is issued on create and immutable afterwards;
// `created_at` / `updated_at` are stamped by the repo; `sort_order` defaults
// to `nextSortOrder` when omitted) are stripped here so the create/update
// bodies cannot dictate them. The remaining required-field list rides
...
export const wireToRecord = (
  ...
  meta: { id: string; sortOrder: number; createdAt: string; updatedAt: string },
): ModelAliasRecord => ({
  id: meta.id,
  name: wire.name,
  kind: wire.kind,
